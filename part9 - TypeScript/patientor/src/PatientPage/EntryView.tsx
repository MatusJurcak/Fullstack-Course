...
const Symbol = ({ entry } : { entry : Entry }) => {
  switch (entry.type) {
    case "OccupationalHealthcare":
      return (<>
        <Icon size="big" color="teal" name='stethoscope'/>
        <strong>{entry.employerName}</strong>
      </>);
    case "Hospital":
      return <Icon size="big" color="red" name='hospital'/>;
    case "HealthCheck":
      return <Icon size="big" color="blue" name='user md'/>;
    default:
...
const HealthcheckDetails = ({ entry } : { entry : HealthCheckEntry }) => {
  if (entry.healthCheckRating === 4) {
    return <Icon name='bell' color='red' />;
  }

  const color = entry.healthCheckRating === 1 ? 'green' : 
                entry.healthCheckRating === 2 ? 'yellow' : 'red';
